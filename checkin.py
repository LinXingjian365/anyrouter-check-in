...
def generate_balance_hash(balances):
    """生成余额数据的hash"""
    if not balances:
        return 'empty'  # 固定值，避免因空数据导致hash变化
...
def retry_call(func, retries=3, delay=2, *args, **kwargs):
    """通用重试函数，返回最后一次结果（若全部失败则返回 None）"""
    last_result = None
    for attempt in range(retries):
        try:
            result = func(*args, **kwargs)
            # 如果结果是 dict 且包含 success 字段，检查是否成功
...
async def check_in_account(account: AccountConfig, account_index: int, app_config: AppConfig):
    ...
    try:
        ...
        user_info_before = retry_call(get_user_info, 3, 2, client, headers, user_info_url)
        ...
        if provider_config.needs_manual_check_in():
            success = retry_call(execute_check_in, 3, 2, client, account_name, provider_config, headers)
            if success is None:
                success = False
            user_info_after = retry_call(get_user_info, 3, 2, client, headers, user_info_url)
